...
class User extends \Opencart\System\Engine\Model {
	...
	public function addUser(array $data): int {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "user` SET `username` = '" . $this->db->escape((string)($data['username'] ?? '')) . "', `user_group_id` = '" . (int)($data['user_group_id'] ?? 0) . "', `password` = '" . $this->db->escape(password_hash(html_entity_decode((string)($data['password'] ?? ''), ENT_QUOTES, 'UTF-8'), PASSWORD_DEFAULT)) . "', `firstname` = '" . $this->db->escape((string)($data['firstname'] ?? '')) . "', `lastname` = '" . $this->db->escape((string)($data['lastname'] ?? '')) . "', `email` = '" . $this->db->escape((string)($data['email'] ?? '')) . "', `image` = '" . $this->db->escape((string)($data['image'] ?? '')) . "', `status` = '" . (bool)(isset($data['status']) ? $data['status'] : 0) . "', `date_added` = NOW()");
	...
	public function editUser(int $user_id, array $data): void {
		$this->db->query("UPDATE `" . DB_PREFIX . "user` SET `username` = '" . $this->db->escape((string)($data['username'] ?? '')) . "', `user_group_id` = '" . (int)($data['user_group_id'] ?? 0) . "', `firstname` = '" . $this->db->escape((string)($data['firstname'] ?? '')) . "', `lastname` = '" . $this->db->escape((string)($data['lastname'] ?? '')) . "', `email` = '" . $this->db->escape((string)($data['email'] ?? '')) . "', `image` = '" . $this->db->escape((string)($data['image'] ?? '')) . "', `status` = '" . (bool)(isset($data['status']) ? $data['status'] : 0) . "' WHERE `user_id` = '" . (int)$user_id . "'");

		if (!empty($data['password'])) {
			$this->db->query("UPDATE `" . DB_PREFIX . "user` SET `password` = '" . $this->db->escape(password_hash(html_entity_decode((string)$data['password'], ENT_QUOTES, 'UTF-8'), PASSWORD_DEFAULT)) . "' WHERE `user_id` = '" . (int)$user_id . "'");
		}
	}
