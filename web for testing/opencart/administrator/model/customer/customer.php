...
class Customer extends \Opencart\System\Engine\Model {
	...
	public function addCustomer(array $data): int {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "customer` SET `store_id` = '" . (int)($data['store_id'] ?? 0) . "', `customer_group_id` = '" . (int)($data['customer_group_id'] ?? 0) . "', `firstname` = '" . $this->db->escape((string)($data['firstname'] ?? '')) . "', `lastname` = '" . $this->db->escape((string)($data['lastname'] ?? '')) . "', `email` = '" . $this->db->escape((string)($data['email'] ?? '')) . "', `telephone` = '" . $this->db->escape((string)($data['telephone'] ?? '')) . "', `custom_field` = '" . $this->db->escape(isset($data['custom_field']) ? json_encode($data['custom_field']) : json_encode([])) . "', `newsletter` = '" . (isset($data['newsletter']) ? (bool)$data['newsletter'] : 0) . "', `password` = '" . $this->db->escape(password_hash(html_entity_decode((string)($data['password'] ?? ''), ENT_QUOTES, 'UTF-8'), PASSWORD_DEFAULT)) . "', `status` = '" . (isset($data['status']) ? (bool)$data['status'] : 0) . "', `safe` = '" . (isset($data['safe']) ? (bool)$data['safe'] : 0) . "', `date_added` = NOW()");
