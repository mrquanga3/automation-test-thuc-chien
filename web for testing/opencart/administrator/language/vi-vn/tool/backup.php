<?php
// Heading
$_['heading_title']      = 'Sao Lưu &amp; Phục Hồi';

// Text
$_['text_success']       = 'Thành công: Bạn đã chỉnh sửa cơ sở dữ liệu thành công!';
$_['text_backup']        = 'Đang sao lưu bảng %s từ bản ghi %s đến %s';
$_['text_restore']       = 'Đang phục hồi %s trong %s';
$_['text_option']        = 'Tùy chọn sao lưu';
$_['text_history']       = 'Lịch sử sao lưu';
$_['text_progress']      = 'Tiến trình';
$_['text_import']        = 'Với tệp sao lưu lớn, hãy tải tệp SQL lên thư mục <strong>~/storage/backup/</strong> qua FTP.';

// Column
$_['column_filename']    = 'Tên tệp';
$_['column_size']        = 'Kích thước';
$_['column_date_added']  = 'Ngày thêm';
$_['column_action']      = 'Hành động';

// Entry
$_['entry_progress']     = 'Tiến trình';
$_['entry_export']       = 'Xuất';

// Error
$_['error_permission']   = 'Cảnh báo: Bạn không có quyền chỉnh sửa Sao Lưu &amp; Phục Hồi!';
$_['error_export']       = 'Cảnh báo: Bạn phải chọn ít nhất một bảng để xuất!';
$_['error_table']        = 'Bảng %s không nằm trong danh sách cho phép!';
$_['error_file']         = 'Không tìm thấy tệp!';
$_['error_directory']    = 'Không tìm thấy thư mục!';
$_['error_not_found']    = 'Lỗi: Không tìm thấy tệp %s!';
$_['error_headers_sent'] = 'Lỗi: Header đã được gửi!';
$_['error_upload_size']  = 'Tệp tải lên không được lớn hơn %s!';