...
{
    // XML-RPC params luôn là mảng bọc ngoài
    ...
    // LOG ĐÚNG
    error_log('XMLRPC ARGS: ' . print_r($this->args, true));

    if (
        isset($this->args[self::$testProjectNameParamName]) &&
        !isset($this->args[self::$testProjectIDParamName])
    ) {
        $name = trim($this->args[self::$testProjectNameParamName]);

        $rows = $this->tprojectMgr->get_by_name($name);
        if (is_array($rows) && !empty($rows)) {
            $info = current($rows);   // ✅ giờ chắc chắn là array
...
protected function _isDevKeyPresent()
{
    ...
    // XML-RPC params dạng: [ { devKey => 'xxxx' } ]
    if (isset($args[0]) && is_array($args[0])) {
        $args = $args[0];
    }

    return isset($args[self::$devKeyParamName]) 
        && trim($args[self::$devKeyParamName]) !== '';
}
